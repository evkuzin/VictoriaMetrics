...
* FEATURE: [vmalert-tool](https://docs.victoriametrics.com/victoriametrics/vmalert-tool/): add `-queryTimeout` command-line flag for limiting the duration of a single rule evaluation. Previously a datasource which accepted the connection but never responded made the unit test hang forever. The timeout is disabled by default.
